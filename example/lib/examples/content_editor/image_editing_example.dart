...
///
/// Demonstrates how to edit and insert images in PDF documents.
///
/// This example shows:
/// - Entering image editing mode in a PDF document
/// - Programmatically inserting new images using Base64-encoded data
/// - Positioning images at specific coordinates on a page
///
/// Key classes/APIs used:
/// - [CPDFEditType.image]: Edit type for image content editing
/// - [CPDFEditManager.changeEditType]: Method to enter image editing mode
/// - [CPDFDocument.createNewImageArea]: Creates a new image area at specified position
/// - [CPDFImageData.fromBase64]: Creates image data from Base64-encoded string
///
/// Usage:
/// 1. Open the example to view the PDF in content editor mode
/// 2. Select 'Enter Image Editing' to enable image editing mode
/// 3. Select 'Insert Image' to add a new image at position (330, 800)
class ImageEditingExample extends StatelessWidget {
  /// Constructor
  const ImageEditingExample({super.key});
  ...
  Widget build(BuildContext context) {
    return ExampleDocumentLoader(
      ...
      builder: (path) => _ImageEditingPage(documentPath: path),
    );
  }
}

class _ImageEditingPage extends ExampleBase {
  const _ImageEditingPage({required super.documentPath});

  @override
  State<_ImageEditingPage> createState() => _ImageEditingPageState();
}

class _ImageEditingPageState extends ExampleBaseState<_ImageEditingPage> {
  ...
  @override
  String get pageTitle => 'Image Editing';
  ...
  void handleMenuAction(String action, CPDFReaderWidgetController controller) {
    switch (action) {
      case 'Enter Image Editing':
        _changeEditType(controller, CPDFEditType.image);
        break;
      case 'Insert Image':
        _insertImage(controller);
        break;
    }
  }
  ...
  Future<void> _insertImage(CPDFReaderWidgetController controller) async {
    final base64ImageData = CPDFImageData.fromBase64(
    ...
    final result = await controller.document.createNewImageArea(
      pageIndex: 0,
      imageData: base64ImageData,
      offset: const Offset(330, 800),
    );
    debugPrint('Insert image result: $result');
  }
}
